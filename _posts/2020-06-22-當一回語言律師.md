...
title: "當一回語言律師"
subtitle: 
description: "上一篇說到宏的 `#` 和 `##` 運算子、一些瑣碎規則和預先定義的宏。有沒有想過這些規則是從哪裏來？老師說的？書本說的?網站說的？他們就一定對嗎？要不來試試當一回語言律師吧！"
...
預處理指令由滿足以下約束的一系列預處理令牌組成：該序列中的第一個令牌是＃預處理令牌，（在翻譯階段4的開始）它是源文件中的第一個字符（可選在不包含換行符的空白之後）或緊隨包含至少一個換行符的空格的空格。 序列中的最後一個標記是序列中第一個標記之後的第一個換行字符。<sup>144</sup> 換行符結束預處理指令，即使該指令出現在本應調用類似函數的宏中。
...
2 字符串文字是沒有前綴的字符串文字。如果在替換列表中，參數緊跟在 `＃` 預處理標記之前，則兩個參數都將被單個字符串文字預處理標記替換，該字符串包含相應參數的預處理標記序列的拼寫。**參數的預處理標記之間每次出現空格都將成為字符串文字中的單個空格字符。刪除第一個預處理令牌之前和最後一個包含參數的預處理令牌之後的空白。否則，參數中每個預處理標記的原始拼寫將保留在字符串文字中，除非進行特殊處理以生成字符串文字和字符文字的拼寫：在字符文字的每個 `“` 和 `”` 字符之前插入一個 `\` 字符或字符串文字（包括定界 `“”` 字符）。**如果替換結果不是有效的字符串文字，則該行為未定義。對應於空參數的字符串文字是`“”`。 `＃` 和 `##` 運算符的評估順序未指定。
...
2 如果在類似函數的宏的替換列表中，參數緊跟在 `##` 預處理標記之前或之後，則該參數將被相應參數的預處理標記序列替換；但是，如果參數不包含預處理 Token，則該參數將替換為地標預處理令牌。<sup>151</sup>

151) 地標預處理標記不會出現在語法中，因為它們是僅存在於翻譯階段4中的臨時實體。
